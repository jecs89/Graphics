...
int main(void)
{
	...
	sphere.setModelMatrix(scale(sphere.getModelMatrix(), vec3(0.1, 0.1, 0.1)));
	...
	moon.setModelMatrix(scale(moon.getModelMatrix(), vec3(0.02, 0.02, 0.02)));
	...
	do
	{
		...
		sphere.setModelMatrix(translate(sphere.getModelMatrix(), vec3( double(r2*std::sin(coordx)) , 0, 0)));
